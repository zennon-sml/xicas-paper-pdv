...
export default function SaleCompletion({
  ...
  useEffect(() => {
    if (props.length > 0) {
      ...
      setTotalQtd((prev) => prev + prop.quantity_sold);
      setTotalDiscount((prev) => prev + prop.discount);
      setTotalValue((prev) => prev + (prop.quantity_sold * prop.price_sold - prop.discount));
    }
  }, [props]);
